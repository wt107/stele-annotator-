...
- **[功能增强]** `_split_stele_paragraphs()` 改为通用匹配，支持多种碑帖的段落拆分
- **[功能增强]** 扩展 Unicode 汉字检测范围，支持扩展 A-G 区汉字（如生僻字、异体字）
- **[性能优化]** 多字典共有字计算改用集合交集，性能从 O(n²) 提升到 O(n)
- **[代码质量]** 添加 `_is_cjk_char()` 辅助函数，统一汉字检测逻辑
